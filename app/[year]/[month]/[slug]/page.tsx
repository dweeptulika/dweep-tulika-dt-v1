...
export async function generateStaticParams() {
  ...
  return articles.map(a => { const m = a.url.match(/^\/(\d{4})\/(\d{2})\/(.+)\.html$/); return m ? { year: m[1], month: m[2], slug: m[3] } : null; }).filter(Boolean) as {year:string;month:string;slug:string}[];
}
...
export default async function Article({params}:{params:Promise<{year:string;month:string;slug:string}>}) {
  ...
  return <div className="container"><div className="breadcrumbs"><Link href="/">Home</Link> / <Link href={"/category/"+categoryPath(article.category)}>{article.category}</Link></div><article className="article"><div className="kicker">{article.labels.slice(0,4).join(" · ") || "News"}</div><h1>{article.title}</h1>{article.excerpt && <p className="dek">{article.excerpt}</p>}<div className="meta">By {article.author || "Dweep Tulika"} · {new Date(article.publishedAt).toLocaleDateString("en-IN",{dateStyle:"long"})}</div>{article.featuredImage && <div className="articleFeaturedImage"><Image src={article.featuredImage} alt="" fill sizes="(max-width: 900px) 100vw, 900px" /></div>}<div className="articlebody" dangerouslySetInnerHTML={{__html:article.bodyHtml}} /><script type="application/ld+json" dangerouslySetInnerHTML={{__html:JSON.stringify(articleJsonLd(article as never,url))}} /><script type="application/ld+json" dangerouslySetInnerHTML={{__html:JSON.stringify(breadcrumbJsonLd(article as never,url))}} /></article></div>;
}
